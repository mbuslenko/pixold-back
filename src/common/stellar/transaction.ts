...
export const sendTransactionToPixold = async (
	...
	try {
		server = new StellarSdk.Server(STELLAR_API_URL);
		issuerKeys = StellarSdk.Keypair.fromSecret(STELLAR_ISSUER_SECRET_KEY);
		senderKeys = StellarSdk.Keypair.fromSecret(sourceSecretKey);
	} catch (e) {
		throw new BadRequestException(`Invalid wallet keys provided `);
	}

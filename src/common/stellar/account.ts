...
export const getAccountInfo = async (publicKey: string): Promise<StellarNamespace.Account.Info> => {
  const response: StellarNamespace.Account.StellarResponse.AccountInfo = await axios.request({
    method: 'GET',
...
const parseBalances = (balances: StellarNamespace.Account.StellarResponse.BalancesEntity[]): { usd: number, xlm: number } => {
  const result = {
    xlm: 0,
    usd: 0,
  }

...
export class WalletService {
	...
	async connectWallet(props: ConnectWalletDto) {
		...
		if (!row) {
			...
			const [wallet] = await this.walletRepository.getWallet(props.userId);
	...
	async sendCoinsToUser(userId: string, coins: number): Promise<void> {
		...
		const [pixoldWallet] = await this.walletRepository.getWallet(
			'6370046c-ac93-4d19-a55f-c59c5d5211b3',
		);

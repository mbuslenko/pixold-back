...
export class PixelService {
  ...
  async changeType(numericId: number, type: 'attack' | 'miner' | 'defender', userId: string): Promise<void> {
    const row = await this.pixelRepository.findOne({ where: { numericId } });

    if (row.ownerId === userId) {
      await this.pixelRepository.update({ numericId }, { type });
    } else {
      throw new BadRequestException({ message: 'You can not change type of other user hexagon' });
    }
  }

  async getHexagonInfo(numericId: number, userId: string): Promise<PixelService.HexagonInfo> {
    ...
    const ownerUsername = (await this.userDomain.getUserById(hexagonRow.ownerId)).username;
